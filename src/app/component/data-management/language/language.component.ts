...
export class LanguageComponent implements OnInit {
	...
	ngOnInit() {
		...
		Promise.all(promises).then(() => {
			//get user setting
			let userSetting = this.common.getUserSettingForPage(this.currentUserSetting, this.config.userSettingKey.page.languageManagement);
	...
	private async editRecord(){
		let data = new Languages(this.selection._selected[0]);
		const dialogRef = this.dialog.open(CommonDialogComponent, {
			width: '500px',
			data: { 
				title: 'Edit language'

...
export class GrammarComponent implements OnInit {
	...
	ngOnInit() {
		...
		Promise.all(promises).then(()=>{
			//get user setting
			let userSetting = this.common.getUserSettingForPage(this.currentUserSetting, this.config.userSettingKey.page.grammarManagement);
	...
	onFilter(event: any) {
		if ((event.which == 13 || event.code == "Enter") && this.searchWord != null) {
			...
			this.currentUserSetting = this.common.updateUserSetting(this.currentUserSetting, this.config.userSettingKey.page.grammarManagement, this.config.userSettingKey.searchWord, this.searchWord);
